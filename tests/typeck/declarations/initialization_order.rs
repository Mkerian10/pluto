...
#[test]
fn var_before_init() { compile_should_fail_with(r#"fn main(){let y=x
let x=1}"#, "undefined variable 'x'"); }

// Class field init order - correctly detected
#[test]
#[ignore] // source is a one-liner the newline healer cannot fix (struct literal split); rewrite when revisiting
...
#[test]
fn closure_capture_before_init() { compile_should_fail_with(r#"fn main(){let f=()=>x
let x=1}"#, "undefined variable 'x'"); }
