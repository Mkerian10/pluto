...
#[test]
fn negate_nullable() { compile_should_fail_with(r#"fn main(){let x:int?=5
let y=-x}"#, "cannot negate type int?"); }
#[test]
fn not_nullable() { compile_should_fail_with(r#"fn main(){let x:bool?=true
let y=!x}"#, "cannot apply '!' to type bool?"); }
