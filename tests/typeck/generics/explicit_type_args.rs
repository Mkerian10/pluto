...
#[test]
fn arg_type_mismatch() { compile_should_fail_with(r#"fn id<T>(x:T)T{return x} fn main(){id<int>(true)}"#, "argument 1 of 'id': expected int, found bool"); }
#[test]
fn return_type_mismatch() { compile_should_fail_with(r#"fn id<T>(x:T)T{return x} fn main(){let s:string=id<int>(42)}"#, "type mismatch"); }
#[test]
fn two_params_first_mismatch() { compile_should_fail_with(r#"fn pair<T,U>(x:T,y:U)T{return x} fn main(){pair<int,bool>(true,42)}"#, "argument 1 of 'pair': expected int, found bool"); }
#[test]
fn two_params_second_mismatch() { compile_should_fail_with(r#"fn pair<T,U>(x:T,y:U)U{return y} fn main(){pair<int,string>(42,42)}"#, "argument 2 of 'pair': expected string, found int"); }
...
#[test]
fn class_too_few_args() { compile_should_fail_with(r#"class Pair<T,U>{first:T
second:U}
fn main(){let p=Pair<int>{first:1, second:true}}"#, "expects 2 type arguments, got 1"); }
#[test]
...
#[test]
fn explicit_conflicts_inferred() { compile_should_fail_with(r#"fn id<T>(x:T)T{return x} fn main(){let x:int=id<string>(42)}"#, "argument 1 of 'id': expected string, found int"); }
#[test]
...
#[test]
fn nested_explicit_outer() { compile_should_fail_with(r#"class Box<T>{value:T} fn wrap<U>(x:U)Box<U>{return Box<U>{value:x}} fn main(){wrap<int>(true)}"#, "argument 1 of 'wrap': expected int, found bool"); }
#[test]
...
#[test]
fn explicit_nullable_mismatch() { compile_should_fail_with(r#"fn id<T>(x:T?)T?{return x} fn main(){id<int>(true)}"#, "argument 1 of 'id': expected int?, found bool"); }

// Explicit args with errors
#[test]
fn explicit_error_mismatch() { compile_should_fail_with(r#"error E{}
fn f<T>(x:T)T{if true{raise E{}}
return x}
fn main(){let s:string=f<int>(42) catch e {"x"}}"#, "type mismatch"); }
