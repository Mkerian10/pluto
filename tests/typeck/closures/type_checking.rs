...
#[test]
fn multi_param_type_mismatch() { compile_should_fail_with(r#"fn main(){let f=(x:int,y:string)=>x
f(1,2)}"#, "expected string, found int"); }
...
#[test]
fn nested_type_mismatch() { compile_should_fail_with(r#"fn main(){let f=()=>(x:int)=>"hi"
let g:fn() fn(int) int=f}"#, "type mismatch"); }
