...
#[test]
fn requires_old_value() { compile_should_fail_with(r#"class C{x:int
fn set(mut self,v:int)
requires old(self.x)>0 {self.x=v}
}
...
#[test]
fn old_undefined_field() { compile_should_fail_with(r#"class C{x:int
fn set(mut self,v:int)
requires old(self.y)>0 {self.x=v}
}
fn main(){}"#, "expected newline after statement"); }

// Old value in invariant
#[test]
fn old_in_invariant() { compile_should_fail_with(r#"class C{x:int invariant self.x>old(self.x)} fn main(){}"#, "expected newline after statement"); }
...
#[test]
fn old_local() { compile_should_fail_with(r#"fn f(x:int)int{let y=x ensures y>old(y) return y+1} fn main(){}"#, "expected newline after statement"); }

// Multiple old references
#[test]
fn multiple_old() { compile_should_fail_with(r#"class C{x:int y:int
fn swap(mut self) ensures self.x==old(self.y) and self.y==old(self.x) {let t=self.x self.x=self.y self.y=t}
}
fn main(){}"#, "expected newline after statement"); }
...
#[test]
fn old_requires_ensures() { compile_should_fail_with(r#"class C{x:int
fn update(mut self,v:int)
requires old(self.x)>0 ensures self.x>old(self.x) {self.x=v}
}
fn main(){}"#, "expected newline after statement"); }
