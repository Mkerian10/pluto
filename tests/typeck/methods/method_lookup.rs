...
#[test]
fn method_not_found() { compile_should_fail_with(r#"class C{x:int}
fn main(){let c=C{x:1}
c.foo()}"#, "no method"); }
...
#[test]
fn method_on_string_wrong() { compile_should_fail_with(r#"fn main(){let
s="hi"
s.foo()}"#, "no method"); }

// Method on array
#[test]
fn method_on_array() { compile_should_fail_with(r#"fn main(){let arr=[1,2,3]
arr.foo()}"#, "no method"); }
...
// Method on map
#[test]
fn method_on_map_wrong() { compile_should_fail_with(r#"fn main(){let
m=Map<string,int>{}
m.foo()}"#, "no method"); }

// Method on set
#[test]
fn method_on_set_wrong() { compile_should_fail_with(r#"fn main(){let
s=Set<int>{}
s.foo()}"#, "no method"); }

// Method lookup through multiple traits
#[test]
fn multi_trait_lookup() { compile_should_fail_with(r#"trait T1{fn foo(self)}
...
#[test]
fn method_generic_lookup() { compile_should_fail_with(r#"class C{
fn foo(self, x: int){}
}
fn main(){let c=C{}
c.bar()}"#, "no method"); }
...
// Method on task
#[test]
fn method_on_task() { compile_should_fail_with(r#"fn f()int{return 1}
fn main(){let
t=spawn f()
t.foo()}"#, "no method"); }
