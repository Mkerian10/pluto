...
- [x] **Cross-entry-point communication** — Resolved in [distributed-model.md](distributed-model.md): stages communicate across logical domain boundaries (`at` placement expressions, channels); the compiler and deployment plan synthesize the physical transport. RPC is one possible lowering, never the model.
